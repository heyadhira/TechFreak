        ...
        <AdminLayout currentPage="AdminServices" title="Manage Services">
            <div className="bg-white rounded-2xl shadow-sm">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                    <p className="text-slate-600">Add and manage your service offerings</p>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {formData.features.map((feature, i) => (
                                            <span key={i} className="px-3 py-1 bg-slate-100 rounded-full text-sm flex items-center gap-2">
                                                {feature}
                                                <button type="button" onClick={() => removeFeature(i)} className="text-slate-500 hover:text-red-500">×</button>
                                            </span>
                                        ))}
                                    </div>
                                </div>
                            ...
                            {isLoading ? (
                                ...
                                services.map((service) => (
                                    <tr key={service.id}>
                                        ...
                                        <td className="px-6 py-4 text-slate-600">₹{service.price_starting?.toLocaleString('en-IN')}</td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${service.is_active !== false ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-700'}`}>
