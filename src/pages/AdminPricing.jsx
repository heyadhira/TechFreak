...
export default function AdminPricing() {
    ...
    const createMutation = useMutation({
        mutationFn: (data) => base44.entities.PricingPlan.create(data),
        onSuccess: () => {
    ...
    const updateMutation = useMutation({
        mutationFn: ({ id, data }) => base44.entities.PricingPlan.update(id, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['admin-pricing'] });
            setIsOpen(false);
            resetForm();
    ...
    const deleteMutation = useMutation({
        mutationFn: (id) => base44.entities.PricingPlan.delete(id),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['admin-pricing'] });
        ...
        <AdminLayout currentPage="AdminPricing" title="Manage Pricing Plans">
            <div className="bg-white rounded-2xl shadow-sm">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                    <p className="text-slate-600">Set up your pricing packages</p>
                                    ...
                                    <div className="space-y-2 max-h-40 overflow-y-auto">
                                        {formData.features.map((feature, i) => (
                                            <div key={i} className="flex items-center gap-2 p-2 bg-slate-50 rounded-lg">
                                                ...
                                                <button type="button" onClick={() => removeFeature(i)} className="text-slate-400 hover:text-red-500">×</button>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
                    {isLoading ? (
                        ...
                        plans.map((plan) => (
                            <div key={plan.id} className={`rounded-xl p-6 relative ${plan.is_popular ? 'bg-gradient-to-br from-indigo-500 to-purple-600 text-white' : 'bg-slate-50'}`}>
                                {plan.is_popular && (
                                ...
                                <div className="absolute top-4 right-4 flex gap-1">
                                    <Button size="icon" variant={plan.is_popular ? "ghost" : "secondary"} className="h-8 w-8" onClick={() => handleEdit(plan)}>
                                        <Pencil className={`w-3 h-3 ${plan.is_popular ? 'text-white' : ''}`} />
                                    </Button>
                                    <Button size="icon" variant={plan.is_popular ? "ghost" : "secondary"} className="h-8 w-8" onClick={() => deleteMutation.mutate(plan.id)}>
                                        <Trash2 className={`w-3 h-3 ${plan.is_popular ? 'text-white' : 'text-red-500'}`} />
                                    </Button>
                                </div>
                                <h3 className={`text-lg font-bold mb-1 ${plan.is_popular ? 'text-white' : 'text-slate-900'}`}>{plan.name}</h3>
                                <p className={`text-sm mb-4 ${plan.is_popular ? 'text-white/80' : 'text-slate-600'}`}>{plan.description}</p>
                                <div className="mb-4">
                                    <span className={`text-3xl font-bold ${plan.is_popular ? 'text-white' : 'text-slate-900'}`}>
                                        ₹{plan.price?.toLocaleString('en-IN')}
                                    </span>
                                    {plan.original_price > 0 && (
                                        <span className={`ml-2 text-lg line-through ${plan.is_popular ? 'text-white/60' : 'text-slate-400'}`}>
                                ...
                                <ul className="space-y-2">
                                    {plan.features?.slice(0, 5).map((feature, i) => (
                                        <li key={i} className="flex items-center gap-2 text-sm">
                                            ...
                                            <span className={plan.is_popular ? 'text-white/90' : 'text-slate-600'}>{feature}</span>
                                        </li>
                                    ))}
                                    {plan.features?.length > 5 && (
                                        <li className={`text-sm ${plan.is_popular ? 'text-white/60' : 'text-slate-500'}`}>
